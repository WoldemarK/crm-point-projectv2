...
public class CustomerWebController {
    ...
    public String getDataForDatatable(@RequestParam Map<String, Object> params) {
        int draw = params.containsKey("draw")
                ? Integer.parseInt(params.get("draw").toString())
                : 1;
        ...
        String sortDir = params.containsKey("order[0][dir]")
                ? params.get("order[0][dir]").toString()
                : "asc";

        Sort.Order sortOrder = new Sort.Order((sortDir.equals("desc")
                ? Sort.Direction.DESC
                : Sort.Direction.ASC), sortName);
        ...
        customers.forEach(customer -> {
            ...
            cellData.put("company", customer.getCompany());
            cells.add(cellData);
        });
